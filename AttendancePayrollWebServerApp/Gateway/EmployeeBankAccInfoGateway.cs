...
namespace AttendancePayrollWebServerApp.Gateway
{
    public class EmployeeBankAccInfoGateway : Gateway
    {
        ...
        public async Task<bool> IsExistUpdateStatusY(string employeeId = "",
                                             int bankCatItemId = 0,int infoId=0,string status="",
                                             string accNo="",string remarks=""
                                             )
        {
            try
            {
                if (employeeId != "" && bankCatItemId != 0 && infoId != 0 )
                {
                    Query = $"SELECT 1 FROM EmployeeBankAccInfo WHERE EmployeeId ='{employeeId}' and BankCatItemId ='{bankCatItemId}' and InfoId <> '{infoId}'  and Status = 'Y'   ";
                    ...
                    Reader.Close();
                    ConnectionClose();
                    if (exist1 == true && status=="N")
                    {
                        accNo = Regex.Replace(accNo.Trim(), @"\s+", "");
                        Query = $" SELECT 1 FROM EmployeeBankAccInfo WHERE InfoId=0";
                        //old4//    Query = $" SELECT 1 FROM EmployeeBankAccInfo WHERE EmployeeId ='{employeeId}' and BankCatItemId ='{bankCatItemId}' and Status = 'N'  ";
                        // old3  Query = $" SELECT 1 FROM EmployeeBankAccInfo WHERE EmployeeId ='{employeeId}' and BankCatItemId ='{bankCatItemId}' and  InfoId <> '{infoId}'  and Status = 'N'  ";
                        //old1 //  Query = $"SELECT 1 FROM EmployeeBankAccInfo WHERE EmployeeId ='{employeeId}' and BankCatItemId ='{bankCatItemId}' and Status ='{status}' and (Remarks ='{remarks}' or Remarks <> '{remarks}' )        ";
                        // old2//Query = $"SELECT 1 FROM EmployeeBankAccInfo WHERE EmployeeId ='{employeeId}'   and BankCatItemId ='{bankCatItemId}' and Status ='{status}' and  Remarks ='{remarks}'     ";
                    }
                }
                Command = new SqlCommand(Query, Connection);
                ConnectionOpen();
