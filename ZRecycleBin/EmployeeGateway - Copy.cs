    ...
    public class EmployeeGateway : Gateway
    {
        ...
        public async Task<Alert> Save(Employee employee, string existCondition = "")
            ...
            try
            {
                ...
                Query = "INSERT INTO Employees (EmployeeId,EmployeeName,DesignationId,CompanyId,DepartmentId,SectionId,SalarySectionId,FloorCatItemId,LineCatItemId,ShiftId,Sex,DateOfBirth,Religion,Blood,MobileNo,EmergencyContact,Mail,EmployeeNameBan,FatherName,MotherName,SpouseName,FatherNameBan,MotherNameBan,SpouseNameBan,PermanentAddress,PermanentAddressBan,PresentAddress,PresentAddressBan,MaritalStatus,NID,BirthCertificateNo,PassportNo,DrivingLicenseNo,BodySign,JoinDate,ReleaseYN,ReleaseDate,Salary,BasicSalary,OverTimeYN,LunchYN,ConfirmYN,ConfirmDate,TransportYN,PaySource,ActiveYN,CardID,SN,EmployeeTypeCatItemId,AppointmentType,HeistEducationCatItemId,ProvidentFundYN,SalaryBuyer,JoiningDateBuyer,BuyerShow,BankName,BankAccNo,BGMEAID,TinNo,RegistrationNo,PhysicalStrength,Experience) VALUES(@employeeId,@employeeName,@designationId,@companyId,@departmentId,@sectionId,@salarySectionId,@floorCatItemId,@lineCatItemId,@shiftId,@sex,@dateOfBirth,@religion,@blood,@mobileNo,@emergencyContact,@mail,@employeeNameBan,@fatherName,@motherName,@spouseName,@fatherNameBan,@motherNameBan,@spouseNameBan,@permanentAddress,@permanentAddressBan,@presentAddress,@presentAddressBan,@maritalStatus,@nID,@birthCertificateNo,@passportNo,@drivingLicenseNo,@bodySign,@joinDate,@releaseYN,@releaseDate,@salary,@basicSalary,@overTimeYN,@lunchYN,@confirmYN,@confirmDate,@transportYN,@paySource,@activeYN,@cardID,@sN,@employeeTypeCatItemId,@appointmentType,@heistEducationCatItemId,@providentFundYN,@salaryBuyer,@joiningDateBuyer,@buyerShow,@bankName,@bankAccNo,@bGMEAID,@tinNo,@registrationNo,@physicalStrength,@experience)";
                ...
                Command.Parameters.AddWithValue("@shiftId", employee.ShiftId);
        ...
        public async Task<Alert> Delete(string empIoyeeId, string condition = "")
        {
            try
            {
                if (condition == "")
                {
                    Query = "DELETE Employees WHERE EmployeeId = @empIoyeeId";
                }
                else
                {
                    Query = "DELETE Employees WHERE " + condition;
                }
        ...
        public async Task<bool> IsExist(string employeeId = "")
        {
            try
            {
                ...
                if(employeeId !="" )
                {
                    Query = $"SELECT 1 FROM Employees WHERE EmployeeId='{employeeId}'";
                }
        ...
        public async Task<Employee> GetEmployee(string empIoyeeId, string condition = "")
        {
            try
            {
                if (condition == "")
                {
                    Query = "SELECT * FROM Employees WHERE EmployeeId = @empIoyeeId";
                }
                else
                {
                ...
                while (Reader.Read())
                {
                    ...
                    employee.ShiftId = (int)Reader["ShiftId"];
        ...
        public async Task<List<Employee>> GetEmployeeListMinCol(string columns, string condition = "")
        {
            try
            {
                ...
                {
                    Query = "SELECT " + columns + " FROM Employees WHERE " + condition;
                }
                ...
                while (Reader.Read())
                {
                    ...
                    SkipOnError(() => employee.ShiftId = (int)Reader["ShiftId"]);
